...
module plus()
{   
//    translate([0,0,0])rotate([0,0,0])cube([20,20,20],true);
    hull()
    {
        translate([66/2-3,0,-40])rotate([0,0,0])cylinder(80,66/2,66/2);
        
        translate([0,0,0])rotate([0,-90,0])cylinder(20,20/2,19.7/2);
    }
    translate([0,0,0])rotate([0,-90,0])cylinder(80,20.3/2,19.7/2);
    
//    rotate([0,90,0])rotate_extrude(convexity = 10, $fn = 100)
//    translate([80, 0, 0])
//        circle(r = 2, $fn = 100);
}
...
module minus()
{
 //   translate([0,0,-10])rotate([0,0,0])cylinder(20,8/2,8/2);
    translate([66/2-3,0,-40])rotate([0,0,0])cylinder(80,55.4/2,55.4/2);
    
    translate([66/2-3,0,33])rotate([0,0,0])cylinder(8,55.4/2,65/2); // top phase
    
    translate([20,0,-25])rotate([0,-90,0])cylinder(60,10/2,10/2);  // cable hole
