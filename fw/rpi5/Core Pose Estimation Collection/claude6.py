...
def _bb_transfer(data: list[int]) -> None:
    ...
    Mode 1 timing per bit:
      1. Clock goes HIGH  (rising edge  - receiver shifts data in)
      2. Half period wait
      3. Data bit is placed on MOSI
      4. Clock goes LOW   (falling edge - receiver samples MOSI)
      5. Half period wait
    """
    cs.off()                              # assert CS (active LOW)
    time.sleep(SPI_HALF_PERIOD)          # short CS setup time

    for byte in data:
        for bit_pos in range(7, -1, -1): # MSB first
            sclk.on()                    # rising edge
            time.sleep(SPI_HALF_PERIOD)
            mosi.value = (byte >> bit_pos) & 1   # data stable in HIGH phase
            sclk.off()                   # falling edge - receiver samples here
            time.sleep(SPI_HALF_PERIOD)
