...
_left_val  = 0    # 0-255  (left arm, unsigned)
_right_val = 0    # 0-255  (right arm, unsigned)
...
def spi_thread():
    ...
    Sends a 2-byte SPI frame every 100 ms:
      byte[0] = left  elbow value as an unsigned byte (0 ... 255)
      byte[1] = right elbow value as an unsigned byte (0 ... 255)

    Arms are distinguished by byte position, not by sign.
    Both bytes are transferred atomically in a single CS-asserted burst.
    ...
    while True:
        ...
        left_byte  = lv & 0xFF           # left  0...255
        right_byte = rv & 0xFF           # right 0...255
